...
var getFolderByType = function (type, cb) {
  nachosSettings.get(function (config) {
    if (!config.packagesPath) {
      return cb(nachosHome('packages', typeFolder[type]));
    }

    cb(path.join(config.packagesPath, 'packages', typeFolder[type]));
  });
};

var getPackage = function (name, type, callback) {
  getFolderByType(type, function(path) {
    var pkgConfig = path.join(path, name, 'nachos.json');
