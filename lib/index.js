...
Packages.prototype._findPackageByName = function (name) {
  ...
  return self.getAll(true)
    .then(function (packages) {
      debug('packages from get all %j', packages);
      var result = _(packages)
...
Packages.prototype.getFolderByType = function (type) {
  if (!type) {
    return Q.reject(new TypeError('type must be provided'));
  }

  if (typeof type !== 'string') {
    return Q.reject(new TypeError('type must be a string'));
  }
...
Packages.prototype.getFolderByPackage = function (pkg) {
  if (!pkg) {
    return Q.reject(new TypeError('package must be provided'));
  }

  if (typeof pkg !== 'string') {
    return Q.reject(new TypeError('package must be a string'));
  }
...
Packages.prototype.getPackage = function (name, type) {
  if (!name) {
    return Q.reject(new TypeError('package name must be provided'));
  }
  ...
  if (type && typeof type !== 'string') {
    return Q.reject(new TypeError('package type must be a string'));
  }
...
Packages.prototype.getDip = function (name) {
  if (!name) {
    return Q.reject(new TypeError('name must be provided'));
  }

  if (typeof name !== 'string') {
    return Q.reject(new TypeError('name must be a string'));
  }
