...
Packages.prototype.getFolderByType = function (type) {
  ...
  if (self._packagesFolder) {
    var join = path.join(self._packagesFolder, typeFolder[type]);

    debug('returning packages folder from local cache: %s', join);

    return Q.resolve(join);
  }
  ...
  return nachosConfig.get()
    .then(function (config) {
      ...
      return path.join(self._packagesFolder, typeFolder[type]);
    });
};
...
Packages.prototype.getFolderByPackage = function (pkg, cb) {
  async.map(Packages.TYPES, this.getFolderByType, function (err, folders) {
    ...
    async.detect(folders, function (folder, callback) {
      fs.exists(path.join(folder, pkg), callback);
    }, function (result) {
      cb(null, result);
    });
  });
